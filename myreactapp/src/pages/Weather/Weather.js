...
function getBgGradient(main = "") {
  ...
  if (m.includes("thunder"))
    return "linear-gradient(135deg,#1a1060 0%,#0d1f15 100%)";
  if (m.includes("rain") || m.includes("drizzle"))
    return "linear-gradient(135deg,#0f2a42 0%,#0d1f15 100%)";
  if (m.includes("snow"))
    return "linear-gradient(135deg,#1a2a3a 0%,#0d1f15 100%)";
  if (
    m.includes("haze") ||
    m.includes("mist") ||
    m.includes("smoke") ||
    ...
    return "linear-gradient(135deg,#2a2010 0%,#0d1f15 100%)";
  if (m.includes("clear"))
    return "linear-gradient(135deg,#0a2a1a 0%,#061410 100%)";
...
function cropHealthScore(weather, aqi) {
  ...
  if (temp > 42) {
    ded += 35;
    risks.push("Extreme heat >42°C — crop damage, soil moisture loss.");
  } else if (temp > 38) {
    ded += 25;
    risks.push("Very high temp >38°C — heat stress, pollen sterility.");
  } else if (temp > 34) {
    ded += 14;
    risks.push("High temp >34°C — moderate heat stress for most crops.");
  } else if (temp > 30) {
    ded += 6;
    risks.push("Warm >30°C — some stress on sensitive crops.");
  } else if (temp < 2) {
    ded += 35;
    risks.push("Frost risk <2°C — severe damage to most crops.");
  } else if (temp < 7) {
    ded += 20;
    risks.push("Cold <7°C — chilling injury risk for tropical crops.");
  } else if (temp < 12) {
    ded += 8;
    risks.push("Cool <12°C — growth slowdown for warm-season crops.");
  } else {
    good.push("Temperature in a favourable range.");
  }
  ...
  if (aqi) {
    if (aqi.aqi === 5) {
      ded += 25;
      risks.push(
        `Air quality Very Poor (AQI 5, PM2.5: ${aqi.pm25.toFixed(1)} µg/m³) — photosynthesis severely blocked.`,
      );
    } else if (aqi.aqi === 4) {
      ded += 16;
      risks.push(
        `Air quality Poor (AQI 4, PM2.5: ${aqi.pm25.toFixed(1)} µg/m³) — reduced photosynthetic rate.`,
      );
    } else if (aqi.aqi === 3) {
      ded += 8;
      risks.push(
        `Air quality Moderate (AQI 3, PM2.5: ${aqi.pm25.toFixed(1)} µg/m³) — mild impact on sensitive crops.`,
      );
    } else {
      good.push(`Air quality ${aqi.label} — minimal crop impact.`);
    }
    if (aqi.pm25 > 150) {
      ded += 10;
      risks.push(
        `PM2.5 critically high (${aqi.pm25.toFixed(1)} µg/m³) — leaf surface deposition.`,
      );
    } else if (aqi.pm25 > 75) {
      ded += 5;
      risks.push(`Elevated PM2.5 (${aqi.pm25.toFixed(1)} µg/m³).`);
    }
    if (aqi.o3 > 120) {
      ded += 6;
      risks.push(`High ozone ${aqi.o3.toFixed(0)} µg/m³ — ozone injury risk.`);
    }
  } else {
    ded += 5;
    risks.push("Air quality data unavailable.");
  }

  // Haze/smoke/dust
  if (desc.includes("smoke")) {
    ded += 15;
    risks.push("Smoke — blocks light, damages leaf surfaces.");
  } else if (desc.includes("dust") || desc.includes("sand")) {
    ded += 12;
    risks.push("Dust/sand — abrades leaf surfaces, blocks stomata.");
  } else if (desc.includes("haze") || main === "haze") {
    ded += 8;
    risks.push("Atmospheric haze — reduced solar radiation.");
  } else if (desc.includes("mist") || desc.includes("fog")) {
    ded += 4;
    risks.push("Fog/mist — reduced light, disease risk.");
  }

  // Visibility
  if (vis !== undefined && vis < 500) {
    ded += 8;
    risks.push(`Very poor visibility ${vis}m — farmwork hazardous.`);
  } else if (vis !== undefined && vis < 2000) {
    ded += 4;
    risks.push(`Poor visibility ${(vis / 1000).toFixed(1)}km — thick haze.`);
  } else if (vis && vis >= 5000) {
    good.push("Good visibility — clear conditions.");
  }

  // Humidity
  if (humidity > 92) {
    ded += 18;
    risks.push("Extremely high humidity >92% — blight/mildew risk.");
  } else if (humidity > 82) {
    ded += 10;
    risks.push("High humidity >82% — elevated fungal pressure.");
  } else if (humidity > 75) {
    ded += 4;
    risks.push("Moderately high humidity — monitor for fungal lesions.");
  } else if (humidity < 20) {
    ded += 18;
    risks.push("Critically low humidity — severe wilting risk.");
  } else if (humidity < 35) {
    ded += 8;
    risks.push("Low humidity <35% — moisture stress.");
  } else {
    good.push("Humidity in acceptable range.");
  }

  // Wind
  if (wind > 20) {
    ded += 14;
    risks.push("Severe wind >20 m/s — lodging, soil erosion.");
  } else if (wind > 13) {
    ded += 8;
    risks.push("Strong wind >13 m/s — tall crop lodging risk.");
  } else if (wind > 8) {
    ded += 3;
    risks.push("Moderate wind — caution with nurseries.");
  } else {
    good.push("Wind speed safe for all crops.");
  }

  // Rain
  if (rain1h > 10) {
    ded += 14;
    risks.push("Very heavy rain — waterlogging/root rot risk.");
  } else if (rain1h > 5) {
    ded += 8;
    risks.push("Heavy rain — field operations suspended.");
  } else if (rain1h > 0) {
    good.push("Light rain providing natural irrigation.");
  }

  // Clouds
  if (clouds > 90) {
    ded += 6;
    risks.push("Dense overcast — photosynthesis significantly reduced.");
  } else if (clouds < 20) {
    good.push("Clear skies — maximum solar radiation.");
  }

  // Thunder
  if (main === "thunderstorm") {
    ded += 10;
    risks.push("Thunderstorm — halt outdoor operations immediately.");
  }
...
function getMeasures(weather, rain24, aqi) {
  ...
  if (aqi?.aqi >= 4) {
    m.push({
      icon: "😷",
      title: "Protect Field Workers",
      tip: "Mandate N95 masks. Limit outdoor exposure to early morning (before 8 AM) when AQI is at its lowest.",
    });
    m.push({
      icon: "🌿",
      title: "Wash Harvested Crops",
      tip: "PM2.5 deposits on leaf surfaces. Rinse all leafy produce thoroughly before sale or storage.",
    });
  }
  if (t > 34) {
    m.push({
      ...
      tip: "Irrigate at dawn and dusk. Avoid midday — over 60% evaporation loss during peak hours.",
    });
    m.push({
      icon: "🌿",
      title: "Apply Organic Mulch",
      tip: "4–6 cm mulch layer lowers root-zone temperature by 8–12°C and reduces moisture loss.",
    });
  }
  if (t < 8) {
    m.push({
      icon: "🏗️",
      title: "Deploy Frost Covers",
      tip: "Use row covers or poly-tunnels overnight. Harvest mature crops before temperatures fall further.",
    });
  }
  if (h > 82 || d.includes("rain")) {
    m.push({
      icon: "🍄",
      title: "Preventive Fungicide",
      tip: "Spray copper-based or systemic fungicide before symptoms appear — humid conditions favour blight and mildew.",
    });
    m.push({
      icon: "🚜",
      title: "Improve Drainage",
      tip: "Clear all drainage channels. Raised beds prevent waterlogging in low-lying fields.",
    });
  }
  if (r > 5 || rain24 > 20) {
    m.push({
      ...
      tip: "Wait 24–48 h post-rain. Wet grain suffers breakage, fungal contamination, and poor storability.",
    });
    m.push({
      icon: "🛑",
      title: "Halt Pesticide Spraying",
      tip: "Rain washes off chemicals — runoff wastes inputs and contaminates water bodies.",
    });
  }
  if (w > 10) {
    m.push({
      icon: "🪢",
      title: "Stake Tall Crops",
      tip: "Stake maize, sunflower and sugarcane to prevent lodging during gusts.",
    });
  }
  if (h < 35) {
    m.push({
      icon: "💧",
      title: "Mist Irrigation for Seedlings",
      tip: "Use micro-sprinklers to maintain nursery humidity >60%. Dry air rapidly desiccates seedlings.",
    });
  }
  m.push({
    icon: "📋",
    title: "Daily Field Scouting",
    tip: "Record pest and disease observations daily — early detection prevents 60–80% of crop losses.",
  });
  m.push({
    icon: "📦",
    title: "Check Stored Grain",
    tip: "Verify grain moisture <14%. Humidity fluctuations accelerate fungal storage damage.",
  });
  return m;
}
...
function makeAnalysis(w, health, rains, aqi) {
  ...
  const aqiTxt = aqi
    ? `Air quality is ${aqi.label} (AQI ${aqi.aqi}, PM2.5: ${aqi.pm25.toFixed(1)} µg/m³) — ${aqi.aqi >= 4 ? "a critical concern for crop photosynthesis and worker health" : aqi.aqi === 3 ? "with mild crop impact" : "posing minimal agronomic risk"}. `
    : "";
  const rainTxt =
    rainIn24 > 10
      ? `Significant rainfall of ${rainIn24.toFixed(1)} mm forecast in 24h (${rainIn48.toFixed(1)} mm over 48h). `
      : rainIn24 > 0
        ? `Light rainfall of ${rainIn24.toFixed(1)} mm possible in 24h. `
        : "No significant precipitation forecast for 24 hours. ";
  const riskTxt =
    health.score >= 80
      ? "conditions are broadly favourable"
      : health.score >= 60
        ? "conditions are acceptable but need monitoring"
        : health.score >= 35
          ? "conditions are concerning and prompt intervention is advised"
          : "conditions are poor — immediate protective action is critical";
  return `Conditions over ${w.name}, ${w.sys.country}: ${Math.round(w.main.temp)}°C (feels ${Math.round(w.main.feels_like)}°C), ${w.weather[0].description}, ${w.main.humidity}% humidity, wind ${w.wind.speed} m/s from ${deg2dir(w.wind.deg)}. ${aqiTxt}${rainTxt}Agronomic outlook: ${riskTxt} (Crop Health Index: ${health.score}/100 — ${health.status}). ${
    health.risks.length
      ? `Priority risks: ${health.risks
          .slice(0, 2)
          .map((r) => r.split("—")[0].trim())
          .join("; ")}. `
      : ""
  }Advised action: ${health.score < 50 ? "implement protective measures immediately" : "maintain vigilance and take precautionary steps"}.`;
}
...
export default function Weather() {
  ...
  const handleSearch = () => {
    ...
    fetchAll(fetchWeatherByCity(city.trim()));
  };
  ...
  const handleLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      (p) =>
        fetchAll(fetchWeatherByCoords(p.coords.latitude, p.coords.longitude)),
  ...
  return (
    <div className="page-container">
      ...
      <div className="wx-search">
        <input
          type="text"
          placeholder="Search city — Delhi, Mumbai, Kathmandu..."
          value={city}
        ...
        <div className="wx-report">
          ...
          <div className="wx-hero" style={{ background: bg }}>
            <div className="wx-hero-left">
              <div className="wx-location">
                ...
                {aqi && (
                  <span
                    className="wx-aqi-badge"
                    style={{
                      color: aqi.color,
                      border: `1px solid ${aqi.color}`,
                      background: aqi.bg,
                    }}
                  >
                    {aqi.emoji} AQI {aqi.aqi} — {aqi.label}
                  </span>
                )}
              </div>
              <div className="wx-temp-row">
                <span className="wx-big-temp">
                  {Math.round(weather.main.temp)}°
                </span>
                <div className="wx-temp-details">
                  <span className="wx-condition">
              ...
              <div className="wx-hero-pills">
                ...
                <span className="wx-pill">
                  💨 {weather.wind.speed} m/s {deg2dir(weather.wind.deg)}
                </span>
                <span className="wx-pill">🌡️ {weather.main.pressure} mb</span>
                <span className="wx-pill">
                  ☁️ {weather.clouds?.all || 0}% cloud
                </span>
                <span className="wx-pill">
                  👁️{" "}
                  {weather.visibility
          ...
          <div className="wx-panel wx-rain-panel">
            ...
            <div className="wx-rain-grid">
              {/* Last Rainfall */}
              <div
                className={`wx-rain-block wx-last-rain wx-last-${lastRain.status}`}
              >
                ...
                <div className="wx-mm-note">
                  *mm = millimetres of rainfall depth
                </div>
              </div>
              ...
              <div className="wx-rain-block">
                ...
                <div
                  className={`wx-rain-pill wx-pill-${rain1h > 5 ? "red" : rain1h > 0 ? "blue" : "muted"}`}
                >
                  {rain1h > 7.5
                    ? "Heavy Rain"
                    : rain1h > 2.5
              ...
              <div className="wx-rain-block">
                ...
                <div
                  className={`wx-rain-block-main ${!rains.firstRain ? "wx-muted" : ""}`}
                >
                  {rains.firstRain
                    ? new Date(rains.firstRain.dt * 1000).toLocaleTimeString(
                        "en-IN",
                ...
                {rains.firstRain && (
                  <div className="wx-rain-block-sub">
                    {new Date(rains.firstRain.dt * 1000).toLocaleDateString(
                      "en-IN",
              ...
              <div className="wx-strip">
                ...
                <div className="wx-strip-row">
                  {forecast.list.slice(0, 8).map((s) => (
                    <div
                      key={s.dt}
                      className={`wx-strip-slot ${(s.rain?.["3h"] || 0) > 0 ? "wx-slot-wet" : ""}`}
                    >
                      <span className="wx-slot-time">
                        {new Date(s.dt * 1000).toLocaleTimeString("en-IN", {
                          hour: "2-digit",
                          minute: "2-digit",
                        })}
                      </span>
                      <span className="wx-slot-icon">
                        {getEmoji(s.weather[0].main)}
                      </span>
                      <span className="wx-slot-temp">
                        {Math.round(s.main.temp)}°
                      </span>
                      {(s.rain?.["3h"] || 0) > 0 && (
                        <span className="wx-slot-rain">
                          {s.rain["3h"].toFixed(1)}mm
                        </span>
                      )}
                    </div>
                  ))}
          ...
          {aqi && (
            <div
              className="wx-panel wx-aqi-panel"
              style={{ borderColor: aqi.color + "44", background: aqi.bg }}
            >
              <h2 className="wx-panel-title">🏭 Air Quality Index</h2>
              <div className="wx-aqi-top">
                <div className="wx-aqi-score" style={{ color: aqi.color }}>
                  {aqi.emoji} AQI {aqi.aqi} — {aqi.label}
                </div>
                <div className="wx-aqi-bar-track">
                  <div
                    className="wx-aqi-bar-fill"
                    style={{
                      width: `${(aqi.aqi / 5) * 100}%`,
                      background: `linear-gradient(90deg,#22c55e,${aqi.color})`,
                    }}
                  />
                  {[1, 2, 3, 4, 5].map((v) => (
                    <div
                      key={v}
                      className="wx-aqi-bar-tick"
                      style={{ left: `${(v / 5) * 100}%` }}
                    />
                  ))}
                </div>
                <div className="wx-aqi-bar-labels">
                  <span>Good</span>
                  <span>Fair</span>
                  <span>Mod.</span>
                  <span>Poor</span>
              ...
              <div className="wx-aqi-pollutants">
                {[
                  ...
                  {
                    label: "PM10",
                    val: aqi.pm10.toFixed(1),
                    u: "µg/m³",
                    warn: aqi.pm10 > 150 ? "High" : "Normal",
                  },
                  {
                    label: "O₃",
                    val: aqi.o3.toFixed(0),
                    u: "µg/m³",
                    warn: aqi.o3 > 120 ? "Crop risk" : "Normal",
                  },
                  {
                    label: "NO₂",
                    val: aqi.no2.toFixed(1),
                    u: "µg/m³",
                    warn: aqi.no2 > 80 ? "Elevated" : "Normal",
                  },
                  {
                    label: "SO₂",
                    val: aqi.so2.toFixed(1),
                    u: "µg/m³",
                    warn: aqi.so2 > 20 ? "Elevated" : "Normal",
                  },
                  {
                    label: "CO",
                    val: (aqi.co / 1000).toFixed(2),
                    u: "mg/m³",
                    warn: "Normal",
                  },
                ].map((p) => (
                  <div key={p.label} className="wx-pollutant">
                    ...
                    <span className="wx-poll-val">
                      {p.val} <small>{p.u}</small>
                    </span>
                    <span
                      className={`wx-poll-status wx-ps-${p.warn === "Normal" ? "ok" : "warn"}`}
                    >
                      {p.warn}
                    </span>
                  </div>
          ...
          <div className="wx-panel wx-chi-panel">
            ...
            <div className="wx-chi-body">
              <div className="wx-chi-left">
                ...
                {aqi && (
                  <div className="wx-chi-aqi" style={{ color: aqi.color }}>
                    {aqi.emoji} Air: {aqi.label}
                  </div>
                )}
              </div>
              <div className="wx-chi-right">
                {health.good.length > 0 && (
                  <div className="wx-chi-section">
                    <div className="wx-chi-section-title green">
                      ✅ Favourable
                    </div>
                    {health.good.map((g, i) => (
                      <div key={i} className="wx-chi-item">
                        {g}
                      </div>
                    ))}
                  </div>
                ...
                {health.risks.length > 0 && (
                  <div className="wx-chi-section">
                    <div className="wx-chi-section-title red">
                      ⚠️ Risk Factors
                    </div>
          ...
          <div className="wx-panel wx-analysis">
            <div className="wx-analysis-header">
              <h2 className="wx-panel-title" style={{ marginBottom: 0 }}>
                📝 Agronomic Analysis
              </h2>
              <div className="wx-analysis-meta">
                <span className="badge badge-green">Live data</span>
                <span className="wx-analysis-time">
            ...
            <div className="wx-disclaimer">
              ⚠️ Always confirm with local field observation and certified
              agronomist advice before critical decisions.
            </div>
          </div>
        </div>
      )}
