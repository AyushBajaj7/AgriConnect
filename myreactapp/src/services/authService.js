...
function getFailureRecord() {
  try {
    return JSON.parse(sessionStorage.getItem(FAILURES_KEY) ?? '{"count":0,"lockedUntil":0}');
  } catch {
...
export function login(username, password) {
  ...
  if (failures.lockedUntil > Date.now()) {
    ...
    return { success: false, message: `Account locked. Try again in ${seconds}s.`, lockedSeconds: seconds };
  }

  const cleanUser = sanitize(username);
  const cleanPass = sanitize(password);
  const isValid   = cleanUser === DEMO_USER.username && cleanPass === DEMO_USER.password;
  ...
  if (!isValid) {
    ...
    sessionStorage.setItem(FAILURES_KEY, JSON.stringify({ count: newCount, lockedUntil }));

    if (shouldLock) {
      return { success: false, message: `Too many failed attempts. Locked for ${LOCKOUT_MS / 1000}s.` };
    }
    const remaining = MAX_FAILURES - newCount;
    return { success: false, message: `Invalid credentials. ${remaining} attempt${remaining !== 1 ? 's' : ''} remaining.` };
  }

  // Successful login — clear failure record, write session
  sessionStorage.removeItem(FAILURES_KEY);
  sessionStorage.setItem(SESSION_KEY, JSON.stringify({ username: cleanUser, loggedIn: true }));
